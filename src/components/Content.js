...
export default function Content() {
        ...
        if (value.shatterEffect !== undefined && !value.emitterConfig) {
          ...
          if (value.emitterConfig) {
            ...
            if (ec.blendMode !== undefined) {
              ec.blendMode = normalizeBlendModeForPixiV8(ec.blendMode);
            }
      ...
      downloadConfig: () => {
        ...
        if (config.particlePredefinedEffect === "shatterEffect") {
          ...
          if (config.emitterConfig?.blendMode !== undefined) {
            downloadableObj.blendMode = normalizeBlendModeForPixiV8(
  ...
  useEffect(() => {
    if (!appReady || !pixiRefs.app?.stage) return;
  ...
  }, [appReady, handleMouseMove, handleResize]);
  ...
  }, [appReady, defaultConfig, fullConfig, handleResize, normalizeRuntimeConfig]);
